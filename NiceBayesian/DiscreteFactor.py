...
class DiscreteFactor(BaseFactor, StateNameMixin):
    ...
    def __init__(self, variables, cardinality, values, state_names={}):  # 这里传进来的values就应该是带梯度的tensor
        """
        ...
        if values.numel() != np.product(cardinality):
        ...
        self.values = values.reshape([i for i in self.cardinality])
        # Set the state names
    ...
    def marginalize(self, variables, inplace=True):
        ...
        phi.values = torch.sum(phi.values, axis=tuple(var_indexes))
    ...
    def reduce(self, values, inplace=True):
        ...
        try:
            ...
            values = [
                (var, self.get_state_no(var, int(state_name))) for var, state_name in values
            ]
        except KeyError:
            warn(
    ...
    def product(self, phi1, inplace=True):
        ...
        if isinstance(phi1, (int, float)):
            ...
            for axis in range(phi.values.ndim):
                ...
                phi1.values = phi1.values.transpose(axis, exchange_index).contiguous()
    ...
    def _str(self, phi_or_p="phi", tablefmt="grid", print_state_names=True):
        ...
        for prob in product(*[range(card) for card in self.cardinality]):
            ...
            prob_list.append(self.values.flatten()[value_index])
    ...
    def __hash__(self):
        ...
        for axis in range(phi.values.ndim):
            ...
            phi.values = phi.values.transpose(axis, exchange_index)
            # phi.values = phi.values.swapaxes(axis, exchange_index)
