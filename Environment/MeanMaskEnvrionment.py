...
# 可以均衡数据
def Mean_data(goal):
    dise_index = defaultdict(list)
    for k, dialog in enumerate(goal):
        dise_index[dialog['disease_tag']].append(k)
    for k, v in dise_index.items():
        print(k, len(v))
    choiced_index = []
    should = 155
    for k, v in dise_index.items():
        need_more = random.sample(v, max(0, should - len(v)))
        x = v + need_more
        choiced_index.extend(x)
    # len(choiced_index)
    train_set = [goal[i] for i in choiced_index]
    return train_set
