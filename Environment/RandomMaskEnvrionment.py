...
class MedicalEnvrionment(gym.Env):
    ...
    def __init__(self, slot_set, start_set, max_turn=22, flag='train', disease_num=4, random_rate=0.1):
    ...
    def reset(self):
        ...
        if self.flag == 'train':
            ...
            r = random.random()
            if r < self.random_rate:  # 在训练集里面引入0.2的噪声
                random_num = random.randint(1, 3)
                random_index = random.sample(list(np.arange(self.disease_num, len(self.slot_dict))), random_num)
                already_index = []
                for x in random_index:
                    if x in im_positive_list or x in im_negative_list:
                        already_index.append(x)
                random_index = list(set(random_index) - set(already_index))
                if 2 * r < self.random_rate:
                    im_positive_list.extend(random_index)
                else:
                    im_negative_list.extend(random_index)
